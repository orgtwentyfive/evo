...
async function main() {
	...
	const { data: categories } = await axios(
		"https://servicii.gov.md/rsspa-api/catalog/categories"
	);
	let i = 0;
	for (const category of categories) {
		...
		const categoryDescription =
			...
			turndownService.turndown(category.description.ro);
		const { data: services } = await axios(
			`https://servicii.gov.md/rsspa-api/catalog/categories/${categoryId}`
		);
		for (const service of services.publicServices) {
			const code = service.code;
			const title = service.title.ro;
			const description =
				service.description.ro &&
				turndownService.turndown(service.description.ro);
	...
	for (const event of events) {
		const { data } = await axios(
			`https://servicii.gov.md/rsspa-api/event/code/${event.code}`
		);
		const title = event.title.ro;
		const description =
			event.description.ro &&
			turndownService.turndown(event.description.ro);
		...
		function parseSteps(node: any) {
			...
			// description
			textSteps += `${turndownService.turndown(
				node.description.ro || ""
			)}\n`;
			// children
...
function processServiceForIndex(data: any) {
	...
	if (data.steps.length) {
		...
		for (const step of data.steps.sort((a, b) => a.order - b.order)) {
			...
			textData += `${turndownService.turndown(
				step.description.ro || ""
			)}\n`;
		}
	}
	...
	if (data.documents.some((d) => d.type === "Required")) {
		...
		for (const document of data.documents) {
			...
			documentText += `${turndownService.turndown(
				document.title.ro || ""
			)}. `;
			if (document.details) {
				documentText += `${turndownService.turndown(
...
function processForGpt(data: any) {
	...
	if (data.steps.length) {
		textData += `# Descrierea procesului de obținere a serviciului în cauză\n`;
		for (const step of data.steps.sort((a, b) => a.order - b.order)) {
			textData += `Pas: ${turndownService.turndown(
				step.title.ro || ""
			)}\n`;
			textData += `Descriere: ${turndownService.turndown(
				step.description.ro || ""
			)}\n`;
		}
	}
	...
	if (data.documents.some((d) => d.type === "Required")) {
		...
		for (const document of data.documents) {
			let documentText = "";
			if (document.type !== "Required") continue;
			documentText += `Doc: ${turndownService.turndown(
				document.title.ro || ""
			)}\n`;
			if (document.details) {
				documentText += `Detalii: ${turndownService.turndown(
					document.details || ""
				)}`;
			}
